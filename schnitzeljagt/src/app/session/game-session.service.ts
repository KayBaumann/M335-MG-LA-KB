...
export class GameSessionService {
    ...
    private recalcTotals(s: GameSession) {
        const totalSchnitzel = s.tasks.reduce((acc, t) => acc + (t.schnitzelEarned || 0), 0);
        const totalPotatoes = s.tasks.reduce((acc, t) => acc + (t.potatoEarned || 0), 0);

        const end = s.finishedAt ?? Date.now();
        const totalDurationSec = Math.max(0, Math.round((end - s.startedAt) / 1000));

        s.totalSchnitzel = totalSchnitzel;
        s.totalPotatoes = totalPotatoes;
